...
public class miniEP3 {
    ...
    public static double Percolates3D(int N, double p, int M) {
        int counter = 0; /* counter para percolação no total de tentativas */
        ...
        for (int count_time = 0; count_time < M; count_time++) {
                ...
                for (int j = 0; j < N; j++) {
                    for (int k = 0; k < N; k++) {
                        if (i % N != N - 1) {
                             ...
                             * Para isso, fazemos: f(x, y, z) = x + n * y + (n^2) * z
                             */
    	    ...
    	    for (int start = 0; start < N*N; start++) {
                for (int end = N*N*N - N*N; end < N*N*N; end++) {
                    if (UnionFind.connected(start, end)) { /* se ha conexao nos extremos do cubo, counter++ */
                        counter++;
                        break;
                    }
                }
